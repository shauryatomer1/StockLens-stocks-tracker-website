...
const WatchlistButton = ({
  ...
  const requestRef = React.useRef<boolean>(!!isInWatchlist);

  // Sync ref with prop if it changes externally
  React.useEffect(() => {
    requestRef.current = !!isInWatchlist;
  ...
  const toggleWatchlist = async () => {
    const shouldAdd = requestRef.current; // Use the intended state from ref
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    setAdded(nextState);
    requestRef.current = nextState; // Update intention immediately
    debouncedToggle();
  };
